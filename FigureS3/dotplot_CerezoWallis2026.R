...
name <- "CellType_1"
src <- "CerezoWallis2026"

df_all <- read.csv(paste0(name, "_AUCell_CerezoWallis2026.csv"), header = TRUE, stringsAsFactors = FALSE)
src_var_names <- colnames(read.csv(paste0(name, "_aucell_acts_CerezoWallis2026.csv"), header = TRUE, nrows = 1, row.names = 1, check.names = FALSE))
...
p <- ggplot(df_scaled, aes(x = source, y = factor(group, levels = rev(sort(unique(group)))),
  ...
  labs(x = src, y = "CellType_1", color = "scaled\nmean diff", size = "-log10(padj)") +
  theme_classic() +
...
ggsave("dotplot_aucell_CerezoWallis2026.pdf", p, width = w, height = h)
write.csv(df_scaled, "dotplot_aucell_CerezoWallis2026.csv", row.names = FALSE)
