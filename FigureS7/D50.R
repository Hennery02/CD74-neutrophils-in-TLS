...
kw_p <- kruskal.test(D50 ~ TLS_group, data = metrics)$p.value
p_label <- if (kw_p < 0.001) "italic(P) < 0.001" else sprintf("italic(P) == %.3g", kw_p)
y_max <- max(metrics$D50, na.rm = TRUE)
y_min <- min(metrics$D50, na.rm = TRUE)
bracket_y <- y_max + 0.14 * (y_max - y_min)
text_y <- y_max + 0.22 * (y_max - y_min)

p <- ggplot(metrics, aes(x = TLS_group, y = D50, color = TLS_group)) +
  ...
  labs(x = NULL, y = "D50 index") +
  theme_tcr()

ggsave("D50.pdf", plot = p, width = 3.6, height = 3.6, device = cairo_pdf)
