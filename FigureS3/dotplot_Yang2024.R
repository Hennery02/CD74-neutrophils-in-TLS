...
df_all <- read.csv(paste0(name, "_AUCell_Yang2024.csv"), header = TRUE, stringsAsFactors = FALSE)
src_var_names <- colnames(read.csv(paste0(name, "_aucell_acts_Yang2024.csv"), header = TRUE, nrows = 1, row.names = 1, check.names = FALSE))
...
ggsave("dotplot_aucell_Yang2024.pdf", p, width = w, height = h)
write.csv(df_scaled, "dotplot_aucell_Yang2024.csv", row.names = FALSE)
