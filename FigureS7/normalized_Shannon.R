...
kw_p <- kruskal.test(normalized_Shannon ~ TLS_group, data = metrics)$p.value
p_label <- if (kw_p < 0.001) "italic(P) < 0.001" else sprintf("italic(P) == %.3g", kw_p)
y_max <- max(metrics$normalized_Shannon, na.rm = TRUE)
y_min <- min(metrics$normalized_Shannon, na.rm = TRUE)
bracket_y <- y_max + 0.14 * (y_max - y_min)
text_y <- y_max + 0.22 * (y_max - y_min)

p <- ggplot(metrics, aes(x = TLS_group, y = normalized_Shannon, color = TLS_group)) +
...
ggsave("normalized_Shannon.pdf", plot = p, width = 3.6, height = 3.6, device = cairo_pdf)
