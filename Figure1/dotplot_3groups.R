...
plot_data <- read.csv("Lineage_C_dotplot_plot_data_3groups.csv", header = TRUE)

custom_order <- c('Subcluster_1',
                   'Subcluster_2',
                   'Subcluster_7',
                   'Subcluster_3',
                   'Subcluster_6',
...
plot_data$contrast <- ordered(plot_data$contrast)

heatmap_layers <- list(
  scale_fill_manual(values = c(scales::muted("#586990"), scales::muted("#cbace4"), "grey10")),
...
p_right <- plot_grid(
  p_odds_ratio + theme(legend.position = "right", plot.margin = margin(t = 0, r = 6, b = 0, l = 0)),
...
ggsave("cluster_dot_plot_3groups.pdf", plot = p_scrna_odds_ratio, width = 7, height = 3.5)
