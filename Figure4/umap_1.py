...
df = pd.read_csv("umap_1_plot_data.csv")
with open("umap_1_cluster1_order.txt") as f:
...
for cat in categories:
    ...
    ax.text(cx, cy, cat, fontsize=10, ha='center', va='center', color='black', weight='bold')

ax.set_xticks([])
ax.set_yticks([])
for spine in ['top', 'right', 'left', 'bottom']:
    ax.spines[spine].set_visible(False)
fig.savefig('umap_cluster_1.pdf', dpi=300, bbox_inches='tight')
plt.close(fig)
