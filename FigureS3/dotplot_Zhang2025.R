...
df_all <- read.csv(paste0(name, "_AUCell_Zhang2025.csv"), header = TRUE, stringsAsFactors = FALSE)
src_var_names <- colnames(read.csv(paste0(name, "_aucell_acts_Zhang2025.csv"), header = TRUE, nrows = 1, row.names = 1, check.names = FALSE))
...
p <- ggplot(df_scaled, aes(x = source, y = factor(group, levels = rev(sort(unique(group)))),
  ...
  labs(x = src, y = "CellType_4", color = "scaled\nmean diff", size = "-log10(padj)") +
  theme_classic() +
...
ggsave("dotplot_aucell_Zhang2025.pdf", p, width = w, height = h)
write.csv(df_scaled, "dotplot_aucell_Zhang2025.csv", row.names = FALSE)
