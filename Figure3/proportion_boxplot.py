...
plot_df = pd.read_csv("cell_proportion_boxplot_plot_data.csv")
TARGETS = plot_df[['cell_type', 'label']].drop_duplicates().itertuples(index=False)
...
for ax, (ct, label) in zip(axes, TARGETS):
    sub = plot_df[plot_df.cell_type == ct]
    grp_data = {g: sub[sub['TLS_group'] == g]['Pct'].values for g in GROUP_ORDER}
    ...
    for i, g in enumerate(grp_keys):
        ...
        ax.scatter(i + 1 + jitter[normal], d[normal],
                   color=GROUP_DARK[g], s=18, alpha=0.60, zorder=2, edgecolors='none')
        if clipped.any():
            ax.scatter(i + 1 + jitter[clipped], np.full(clipped.sum(), y_clip * 0.97),
                       color=GROUP_DARK[g], s=22, alpha=0.80, zorder=2, edgecolors='none', marker='^')
    ...
    for idx, (i, j) in enumerate(pair_list):
        ...
        pstr = pformat_sci(p)
        ax.text((i + j) / 2 + 1, y_line + tick_h * 1.3, pstr, ha='center', va='bottom', fontsize=7.5, color='black')
    ...
    for i, g in enumerate(grp_keys):
        ax.text(i + 1, -y_top * 0.12, f'n={len(grp_data[g])}', ha='center', va='top',
...
plt.tight_layout()
plt.savefig('cell_proportion_boxplot.pdf', bbox_inches='tight')
